...
@WebServlet("/live/getAllChatRoom")
public class GetAllChatRoomServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			String result = HTTPUtil.requestGet(I.REQUEST_BASE_URL_CHATROOM, authorization);
			...
			if(dao.updateAuthorization(token)){//更新数据表中的token
				// 重新发送创建聊天室的请求
				result = HTTPUtil.requestGet(I.REQUEST_BASE_URL_CHATROOM, authorization);
				JsonUtil.writeJsonToClient(result, response);
			}
		} catch (Exception e) {
