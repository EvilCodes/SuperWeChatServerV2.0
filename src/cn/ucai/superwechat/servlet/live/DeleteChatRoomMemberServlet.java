 ...
 * 删除直播室指定成员
 */
@WebServlet("/live/deleteChatRoomMember")
public class DeleteChatRoomMemberServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String result = HTTPUtil.requestDelete(I.REQUEST_BASE_URL_CHATROOM+"/"+chatRoomId+"/users/"+username, authorization);
			...
			if(dao.updateAuthorization(token)){//更新数据表中的token
				// 重新发送创建聊天室的请求
				result = HTTPUtil.requestDelete(I.REQUEST_BASE_URL_CHATROOM+"/"+chatRoomId+"/users/"+username, authorization);
				JsonUtil.writeJsonToClient(result, response);
			}
		} catch (Exception e) {
